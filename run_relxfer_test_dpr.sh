...
if [[ "$1" == *"dev"* ]]; then
    setting=run.miracl.mdprxfer-op-$ckpt_type.lang.dev
    ...
    for lang in ar bn en es fa fi fr hi zh id ja ko ru sw te th;do
        python3 -m pyserini.search.faiss \
          ...
          --encoder checkpoints/mdpr-tied-pft-msmarco-rel-xfer-setting2/checkpoint-$ckpt \
...
if [[ "$1" == *"test-a"* ]]; then
    setting=run.miracl.mdprxfer-op-$ckpt_type.lang.test-a
    mkdir -p runs/$setting
