...
if [[ "$1" == *"dev"* ]]; then
    setting=run.miracl.mdprxfer-med.lang.dev
    ...
    for lang in ar bn en es fa fi fr hi zh id ja ko ru sw te th;do
        python3 -m pyserini.search.faiss \
          ...
          --encoder checkpoints/mdpr-tied-pft-msmarco-rel-xfer-vanilla/checkpoint-3000 \
...
if [[ "$1" == *"test-a"* ]]; then
    setting=run.miracl.mdprxfer-med.lang.test-a
    mkdir -p runs/$setting
